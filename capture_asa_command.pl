...
# Filename: capture_asa_command.pl
...
# Usage: capture_asa_command.pl <device IP> <user> <pass> <enable_pass> <login_timeout_in_seconds> <capture_timeout_in_seconds> <your_command_here>
# Example: capture_asa_command.pl 4.2.2.2 admin mysecret mypassword 3 3 "show interface"
#
# -----------------------------------------------------------------------------------------------
# Known issues:
# 
...
if( $#ARGV != 6 ) {
	print "Usage: capture_asa_command.pl <device IP> <user> <pass> <enable_pass>
			<login_timeout_in_seconds> <capture_timeout_in_seconds> <\"command\">\n";
	print "Example: capture_asa_command.pl 4.2.2.2 admin mysecret mypassword 3 3 \"show interface\"\n";
	print STDERR "Usage:  capture_asa_command.pl <deviceIP> <user> <pass>
...
sub GetConfig {
	...
	my $command=shift;			# $ARGV[6]
	my @config;
	my $msg;
	my $en_command="enable";
	my $term_command="terminal pager line 0";
	my $gconf_command="configure terminal";
	...
	my $gconf = $ssh->exec( $gconf_command );
